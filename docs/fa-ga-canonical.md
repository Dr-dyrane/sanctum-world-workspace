...
Headings as prose beats (decision B). The FA leads with the Output Score (the writer's `Overall Failure Score: X.XX / 1.0` line and a brief failure-framed justification), then What the Agent Got Wrong (most consequential flaw first, anchored to the golden), then Why the Failure Matters (would not be deliverable by a competent clinician). The GA covers Suggested Score (restate the writer's score in one line and name the gap to the grader's score) and Score Comparison (the grader audit, what it got right and wrong, citing the grading summary directly). Do not reference the self-score bands in the prose.
