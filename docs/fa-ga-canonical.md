...
4. The SCORE - state the analyzed run's score and the writer's own score, and tie the failure to them. Reference ONLY this run; the full distribution stays in the Status line, never the body (Abi single-run rule). Conclude the FA with the exact required line: `Overall Score: NN%`. This is Alexander's own score, not the trajectory score.
...
Submit your OWN score of how the model did, alongside the FA. It is the writer's independent assessment of the model's task performance on the 0% to 100% scale, NOT the trajectory score (the agentic grader's number), and it is compared against the agentic grader to calibrate human-versus-grader agreement. Score the run on the task's own logic: the central failure caps it low, give partial credit for the accurate items, no credit for refusing to finish. Judge it independently; do not copy the trajectory number. It usually lands near the grader when the grader is calibrated; when it diverges, the GA's calibration move should explain why. Placement: the writer's score goes in the Studio score field AND inside the submitted FA field, because the Status line is internal and is not pasted into Studio. The FA must LEAD with the exact required line `Overall Score: NN%` (the Output Score, stated first per the 2026-06-20 FA template), with a brief justification. Record it in the Status line too. This is the writer's own number; it is distinct from the banned grader-rating line (Poor / Fair / Good / Great), which never appears in the file, not even the Status line. Bind it to the same run the FA analyzes.
...
Headings as prose beats (decision B). The FA leads with the Output Score (the writer's `Overall Score: NN%` line and a brief failure-framed justification), then What the Agent Got Wrong (most consequential flaw first, anchored to the golden), then Why the Failure Matters (would not be deliverable by a competent clinician). The GA covers Suggested Score (restate the writer's score in one line and name the gap to the grader's score) and Score Comparison (the grader audit, what it got right and wrong, citing the grading summary directly). Do not reference the self-score bands in the prose.
...
FA: leads with the Output Score line `Overall Score: NN%` as the first element, then names the run "On trajectory N", with a brief failure-framed justification (the score is literally first, 2026-06-20); failure-only with NO competent baseline or praise (AutoQC fails any positive framing); names the central failure as a turn, most consequential first, anchored to the golden; states the clinical mechanism and consequence with a severity tier; states plainly it would not be deliverable by a competent clinician; names the specific source documents by author or date (not a generic reference); references only this run, never other runs' scores.
...
Self-check: run `python3 tools/verify/lint_fa_ga.py <file>` and clear every FAIL (it enforces the Output-Score-line-first opener (run name right after), single-run-only, the score-line, a dated source-document citation, and the structure rules; it warns on semicolons and long sentences).
