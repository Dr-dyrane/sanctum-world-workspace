...
Test: read only what the model receives. If the most defensible completion (trusting the draft it was handed, following the file it was given) is the behavior that floors, the construction is unfair. Any mounted draft carrying a false or unverified claim must do one of: attribute it to an unverified source, leave it as a placeholder the model synthesizes, or instruct reconcile-and-correct (knowing that last one kills propagation difficulty).
Source: KM05 (home-BP attribution), KM06 (false closures), KM07 v2 (planted bone-health closure, retired despite flooring 9 of 10). Three instances before it became a class. Treat every new reviewer flag as a class on day one.
...
These are checkable without judgment and catch the cheap failures first: date-anchor scan across every header, footer, table, and signature; banned-character scan (em dash, en dash, arrows); fingerprint and metadata gates per the runbook; prompt-to-file reference scan; workflow string matches the tracker sheet exactly; grader chart-access setting matches the task type; new-to-world names checked for collisions and flagged for confirmation; preregistration locked before any pilot, never edited after.
...
Carried from the live record on 2026-06-11 (PM update): KM07 v3 RETIRED as unfair (quiet bait, Abi catch verified on bytes; record at task7/qa/abi-mode-review-2026-06-11.md); KM07 v4 true-placeholder direction approved, build awaits Alexander authorization, fresh prereg and the A0.5 gate required, golden self-score required before any banking; KM08 fairness-gate finding open for Alexander's decision (draft pre-writes the uptitration order; v5 reshape recommended; record at task8/qa/abi-mode-review-2026-06-11.md); KM10 v2 prompt still carries the 05/26 memo vs "this morning 5/27" date wrinkle (one-word fix available); KM10 v2 needs a fresh locked preregistration before any pilot; KM07 QA tech-issue annotation needs the substantive rebuttal rerun. Remove items from this list only when the underlying artifact closes them.
