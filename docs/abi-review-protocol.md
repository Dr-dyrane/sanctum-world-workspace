...
Test: describe the grader as scoring output against the golden and guidelines, never as independently investigating. QA dispositions need substantive, fact-referenced rebuttals (cite the job, the runs, the observed behavior), never "the reviewer said it's okay" and never the bare words "tech issue." If a QC/Taiga issue is not writer-fixable, route it to the correct technical channel and still write the local note with the concrete cause and evidence. FA/GA is failure-only, natural prose, complete sentences, no bullets, no headers, no section names, bound to the 2nd-LOWEST scoring run in the latest valid run set (King P 2026-06-14 / DO-NOT-REPEAT #20; supersedes the earlier "single lowest run"). If trajectories and QA are rerun, redo FA/GA from the latest run set. Each part stays under about 1000 characters, bound to the selected run in Studio. FA includes Alexander's own score and LEADS with `Overall Failure Score: X.XX / 1.0` (the Output Score first, 2026-06-20 template).
