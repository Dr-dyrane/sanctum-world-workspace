...
- Next eligible phase: Transcript Resolution Review. Do not run AutoQC, create AutoQC responses, create scoring rubrics, scoring thresholds, pass/fail bands, point allocations, DOCX submission artifacts, manifests, final submission packages, uploads, submissions, or RL Studio access until explicitly authorized.
...
- Do not proceed beyond Transcript Resolution Review into AutoQC runs, AutoQC responses, scoring rubrics, scoring thresholds, pass/fail bands, point allocations, DOCX packaging, manifest creation, final submission packaging, upload, submission, or failure analysis without explicit authorization.
