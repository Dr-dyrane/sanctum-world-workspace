...
- Next eligible phase: Reference File Resolution. Do not run AutoQC, create AutoQC responses, create scoring rubrics, scoring thresholds, pass/fail bands, point allocations, DOCX submission artifacts, manifests, final submission packages, uploads, submissions, or RL Studio access until explicitly authorized.
...
- Do not proceed beyond Reference File Resolution into AutoQC runs, AutoQC responses, scoring rubrics, scoring thresholds, pass/fail bands, point allocations, DOCX packaging, manifest creation, final submission packaging, upload, submission, or failure analysis without explicit authorization.
