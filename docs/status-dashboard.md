...
6/8 KM05 reset: KM05 is review-only / HOLD. Start at `worlds/korvin-merrow/task-setup/task5/TASK5-STATE.md` and `task5/design/KM05-v2-design-plan-6-8.md`, then read the older 6/7 review files only as history. The current lead hypothesis is a single unsupported post-discharge BMP / stable renal-potassium monitoring claim inside a transition-clinic draft. The older moderate-task framing is retired by the 6/8 no-moderate directive; future KM05 must target a real clinical failure. No DOCX build, platform-current staging, RL Studio upload, Task AutoQC, Taiga trajectories, QA, or platform mutation.
