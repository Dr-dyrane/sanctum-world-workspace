...
| Model grader's reasoning lists "what was done well" (read as the FA dwelling on positives) | OV08 d4eefce6: the authored FA fields are failure-only, but the selected model grader's runtime rationale (traj_3603df058) lists four positives before the failure | a model grader naturally enumerates strengths in its scoring chain-of-thought; the AutoQC treats that grader reasoning as part of the failure-analysis apparatus | Dispute: the authored FA field is failure-only and the grader's transient per-trajectory rationale is the grader's, not a writer artifact, and is out of scope (paste-ready in docs/qc-dispositions.md D2). To reduce recurrence, do not write grader guidelines that invite a strengths list, then re-pilot. The writer cannot edit a grader's runtime reasoning. |
| Trajectory floor flagged as output-constraint non-compliance | OV08 Trajectory AutoQC flagged the anti-paralysis floor (blank determination) for violating "finalize it for submission" | the trajectory AutoQC "Compliance with Output Constraints" criterion reads a floor trajectory's task-failure as a defect | Justify in the Notes, do not re-run: the non-compliance is the intended, grader-scored failure mode, and the catcher runs prove the constraint is reachable. Paste-ready in docs/qc-dispositions.md (D1). |

Borderline findings (the artifact is correct and the QC is over-reading) are not fixed; their paste-ready disputes live in docs/qc-dispositions.md. Do not re-pilot a healthy task to appease one.
