...
1. **Re-read the LIVE instruction document first.** Pod announcement 6/10 (Larry): "the guidance has changed" for new-world brainstorms. Verify the live doc against the current tracked copy at `reference/source/instruction-doc/[EXP] Project Sanctum Instruction Document (06_09).md` (source-of-truth index: `reference/source/README.md`; prior versions in `reference/source/_superseded/instruction-doc/`); if it moved past 06_09, re-snapshot before proceeding.
...
5. **Claim check.** Verify every intended workflow string verbatim against the live Task Selection Categories sheet (current local copy: `reference/source/task-selection-categories/Sanctum_Task_Selection_Categories_Combined_06_19.docx`; prior snapshots in `reference/source/_superseded/task-selection-categories/`) and note claim counts.
