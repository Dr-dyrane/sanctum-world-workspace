...
- Verdict tiers: plain A / plain B (narrow), then B1/A1 (slightly better), then B2+/A2+ ONLY for a real caught-finding or structural difference. Tier must match the gap: if both handle the mechanism and differ only in clarity/one detail, it is B1/A1, not B2.
