...
- Verdict tiers (official Part 7 doc; we cap at A1-B3). The platform shows the full A4-B4 scale (eight buttons), but per the standing pod call we cap at A1-B3, plain through two plus, and never select three plus (A+++/B+++). Restore the Scale Selection line as the lead pasted line, language only (for example "A++ (moderately better)"); the Part 7 template language is marginally (plain), slightly (one plus), moderately (two plus). Tier must match the gap: if both handle the central item and differ only in clarity or one detail, it is plain, not one plus. Part 7 rules that bind the prose: judge the work product not the trajectory, ignore the grading scores and never cite them in the pasted text, reference the golden and grader content specifically not vaguely, and the writer authors the final text.
