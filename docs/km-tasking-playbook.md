...
- Verdict tiers (official Part 7 doc; full A4-B4, pod-confirmed 2026-06-18). Use the full A4-B4 scale, eight buttons. The pod confirmed the 06/09 instruction doc, the PL template, and the How to Do PLs section all use A4-B4 and there is no documented A1-B3 narrowing; this supersedes the interim A1-B3 cap. Lead the pasted comment with the Scale Selection line, tier plus language as the pod modeled ("A2 (better)"): A1 slightly better (plain), A2 better (one plus), A3 much better (two plus), A4 significantly better (three plus). Mind the button mapping, the tier number is one ahead of the plus count. Conservative margin still governs: a clean catch-versus-floor with an otherwise-competent loser is A3 (two plus); reserve A4 for a multi-error or unsafe output. Part 7 rules that bind the prose: judge the work product not the trajectory, ignore the grading scores and never cite them in the pasted text, reference the golden and grader content specifically not vaguely, and the writer authors the final text.
...
- FA = failure-only, what the model did poorly. It includes Alexander's own score, not the trajectory score, and LEADS with `Overall Failure Score: X.XX / 1.0` (the Output Score first). GA audits how the grader scored that run: what it identified correctly, what it missed, what it mis-scored, and whether calibration holds. No praise paragraph, no grader-section names (state the content), two paragraphs each, ~<=1000 chars, physician voice, NO em/en dashes.
