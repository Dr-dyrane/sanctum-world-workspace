...
4. Bury one critical finding off the text when clinically realistic. Off-text evidence can be harder for the model than prose, but source it safely. A renderer-built report, handwritten list, scanned form, medication list, device printout, or downtime note can carry a decisive finding, especially if the prose only contains indirect hints such as a low-grade temperature or "photo taken." For future or reopened work, do not use AI-generated clinical photographs. If a genuine photo is essential, use a valid public-domain or permissively licensed noncopyrighted real image, or redesign the mechanism.
...
Guardrail: off-text and multimodal traps must still be fair. The image, report, or audio must be visible to the agent, realistic for the workflow, temporally anchored, and supported by at least a minimal chart clue. The model should fail because it did not integrate available evidence, not because the file was hidden, post-cutoff, illegible, impossible to interpret, or sourced from a prohibited AI-generated clinical photo. For guideline-timing traps, the needed rule must match the world's date, or the dated guideline source must be attached.

FRAME CHOICE controls the distribution shape (2026-06-20, OV08 plus OV10 redemption). The deliverable FRAME, completion versus review, decides whether a task floors universally or splits bimodal. A COMPLETION frame (finish or finalize this for signature) primes trust and can floor EVERY run when the catch is an absence or a copied-forward over-attestation: OV10 v1 floored uniformly 0.12 to 0.18 with no catcher, which Larry flags as unfair. The REVIEW-AND-CORRECT frame (review this complete, signed note before co-signing, ready or needs correction) makes auditing the job, so capable reviewers catch the line and rubber-stampers miss it, splitting bimodal: OV10 v2 0.15 to 0.85 and OV08 0.10 to 0.95, the same chart and catch as the floored completion version. For a reliable bimodal on an over-attestation or absence catch, prefer the review-and-correct frame over completion. Full recipe: worlds/ondina-vasquell/docs/FLOOR-MECHANISM-LIBRARY.md (review-and-correct refinement); the fairness mandate is reference/world-spec-guidelines/POLICY-2026-06-20-deprecated-workflows-and-task-fairness.md.
...
## Embedded-wrong floors, suggested-wrong ceilings (canon, 2026-06-14)
Evidence: OV01 banked (enoxaparin embedded in the discharge order set, model must STOP it - floored 0.40/0.50), while OV02 and OV07 ceilinged (bad premise SUGGESTED by an external worksheet/query, model only had to decline). This model is strong at declining a suggested bad claim and at not fabricating; it is weak at catching a wrong element already embedded in the draft it is completing, at acting/normalizing when its default leans that way, at rendering a binding decision instead of hedging, at omitting a required element under load, and at integrating an off-text/image signal.
DESIGN RULE: put the floor-lever as EMBEDDED-WRONG (catch + remove/correct inside the deliverable the model finalizes) or on a commission/omission/off-text axis - NOT as "decline an externally suggested bad premise," which the model reliably handles. Restraint traps in fully-reconciling genres (coding attestation, HEDIS abstraction, CDI decline) are model-strong and will ceiling; do not build the suite around them.

## Floor target = per-task judgment (ratified 2026-06-14)
DEFAULT (design): KM09 shape - the embedded error is the MODAL move so most runs commit it, the grader SCORE-CAPS the central failure as critical (a floor run lands ~0.15-0.30, not a deduction), AND at least one reachable catcher >0.85 proves the floor is fair and the correct answer reachable. This matches the Canonical fix (floor 0.30-0.55 + catcher >0.85) and KM09's actual banked spread (mean 24 WITH two catchers).
EXCEPTION (approval): a fair, legitimate all-floor task may bank WITHOUT a catcher when the failure is clinically material and the construction is fair (King P 6/12) - decided case by case at the pilot, not designed for up front.
DO NOT: design or grade for "all runs <30, no catcher." That over-rotates past KM09, drops the reachability proof, and tips into manufacturing depth at the grader (forbidden). Keep/welcome catchers; they are the task proving itself fair, not failing.
