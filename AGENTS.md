...
3. THE LAW. Build it real; build the task with the world's per-artifact rigor; read every file in full to OUTSMART the model not feed it; difficulty is empirical and adversarial, found by running it. Difficulty equals a FORCED SLOT the model's competence plays wrong; if the genre permits deferral (a discharge summary), manufacture the forced move with a mounted adversarial input; ship the calibrated number, do not soften. FAIRNESS IS THE OTHER HALF (Abi doctrine, paid for on KM05/KM06/KM07): a floor is bankable only if the failure is the model's own fault. NEVER floor a model for propagating a false claim planted in its OWN draft with no instruction to correct it - Abi retired KM07 v2 (9/10 floored) for exactly this. Fix by (a) re-attributing the claim to an unverified source, (b) leaving a placeholder the model synthesizes from the chart, or (c) a reconcile-and-correct prompt instruction - but (c) is a DIFFICULTY-KILLER on propagation mechanisms (KM06 v4 went to ~0.98 all-catch), so reserve it for genuine judgment traps. An adversarial document wrong-BY-GENRE (payer denial, CDI query, pharmacy handoff) needs no fix - it is fairly rebuttable. Confirm a catch is reachable (a catcher run, or the golden scoring high under its own grader). Full doctrine: docs/task-difficulty-lessons.md sections 5-6.
...
CURRENT STATUS (11 June 2026). Of the ten tasks in this world, six have completed first human review and are delivered: KM01 through KM06. The remaining four are under first human review: KM07, KM08, KM09, and KM10. KM07, the nephrology referral, is in its third version, reseeded after the second was returned as unfair because it planted a bone-health closure the model was never asked to correct; the current version leaves the medication-reconciliation status for the model to complete from the record, the grader now reads the chart, and it pilots as a fair mid-range result (mean 0.525, job dba6c34f), with the golden-reachability check still outstanding. KM08 is the gabapentin-uptitration progress note (bimodal, mean ~0.67). KM09 is the coding attestation that declines to anchor the principal diagnosis on sepsis (~0.31). KM10 is the documentation-integrity query response; it pilots uniformly low (~0.25), and we have not yet confirmed that a careful answer can decline the unsupported diagnosis and still score well, which is the item to watch before it is accepted. The authoritative, moment-to-moment status is the platform board and the live records (`dashboard/km-world-dashboard.html`, `worlds/korvin-merrow/task-setup/KM-WORLD-PERFORMANCE-REPORT.md`, and each `taskN/TASKN-STATE.md`); trust those over any narrative here. The client's latest-guidance example, a documentation-integrity query world and task, is mirrored at `reference/templates/` with an orienting README; read it before building a documentation-integrity, coding, or external-query task. No upload, automated QC, agent run, QA response, preference label, review, or platform change proceeds without Alexander's authorization for that specific step.
