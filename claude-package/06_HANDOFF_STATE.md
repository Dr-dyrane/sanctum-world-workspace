...
---
6/8 late Sang grader/golden review fix: KM03 and KM04 are currently AT TAIGA TRAJECTORY RERUN after grader/golden revisions. The prior KM03 `877aa204` and KM04 `709be0e8` runs remain evidence only, and their FA/GA drafts are provisional. New required grader structure/length lesson: `docs/grader-guidelines-lessons.md`. Do not help enter old-run FA/GA, run FA/GA AutoQC, create PL, or start final review for either task until rerun scores/transcripts exist and FA/GA is re-derived from the new lowest run.
