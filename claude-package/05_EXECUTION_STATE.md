...
- For Task 3, start with `worlds/korvin-merrow/task-setup/task3/TASK3-STATE.md` and `worlds/korvin-merrow/task-setup/platform/task3/current/RUN-INSTRUCTIONS.md`. The staged KM03 escalation set contains `prompt-task3-escalation.txt`, `case_management_discharge_readiness_clearance_05242026.docx`, `golden-KM03-v1.docx`, and `grader-guidelines-task3.txt`. Historical design/gate material remains at `task3/design/`, `task3/KM03-3rd-reader-review-and-build-gates.md`, and `task3/build-phase-drafts/`. Do not upload, run AutoQC, run agents, run QA, or mutate RL Studio until Alexander finalizes the writer-owned prompt, signs off the golden clinical content, and Abi greenlights tasking.
...
---
CURRENT STATE 6/7 pm: KM02 COMPLETE through writer workflow, Awaiting Final Review. Both human reviews passed (Abi 6/7), all recorded KM02 checks are green, and Preference Labels were submitted with verdict B / B++. Golden sha 2dd3e0ad. Codex restructure executed (platform/task2 now current/hold/). KM03 uses the COMPLETION-overclaim frame. Codex gate passed and `platform/task3/current/` draft staging is complete; upload stays held pending Alexander prompt finalize, Alexander golden sign-off, and Abi tasking greenlight.
