...
- For Task 4, do not start from memory. First read `project/WORKSPACE_FILE_MAP.md` Navigation Rule, then `worlds/korvin-merrow/task-setup/task4/TASK4-STATE.md` and `worlds/korvin-merrow/task-setup/task4/KM04-prebuild-review-and-build-gates.md`. KM04 remains blocked until the read receipt, KM03 inheritance check, hypothesis kill-chain, clinical-register scan, and difficulty prediction are recorded and Alexander authorizes the exact build step.
