...
## v2 CORRECTION 2026-06-17 - use the EXISTING canonical worksheet (Dyrane: use existing builders, reindex/reread)
The v1 work above invented a new "concurrent stay review" deliverable and an ad-hoc python-docx builder. That was wrong on two counts: (1) it bypassed the canonical Epic renderer (epic.py / build_world_files.build_one), and (2) the UR continued-stay deliverable ALREADY EXISTS as task_data.T6 -> continued_stay_determination_worksheet_05252026.docx, a first-level worksheet that already embeds the antibiotic-route trap as a FALSE entry: "transitioned to oral antibiotics." There is also an existing golden (build_goldens.OV06, stale filename) that caught only on the loud operational barriers and never flagged that antibiotic lie.

v2 uses the existing worksheet as the deliverable (rendered through build_task_files._to_world_spec(task_data.T6())) and authors a corrected golden that makes the antibiotic-route lie load-bearing: the MAR shows ongoing IV vancomycin, piperacillin-tazobactam, and cefepime through 05/21; no oral conversion (sulfa allergy); no OPAT; home-health cannot administer IV antibiotics. FLOOR = adopt the worksheet's discharge determination, carrying the oral-antibiotics claim forward. CATCHER = reject it and hold continued stay on the antibiotic route. The invented concurrent-stay-review draft is deleted. Mechanism reclassified: conflicting-authority / carry-forward hybrid (embedded-wrong worksheet entry), not a pure carry-forward omission.
