...
## Workflow (Step 10)
Acute Care Discharge Planning (P1). Fresh eighth lane. Exact string from the 06/18 Approved Task Selection Categories catalog.

## Prompt
Use prompt-OV12.txt verbatim:
"Dr. Foss started Mrs. Vasquell's discharge plan for her transfer to skilled nursing and got pulled away before finishing it. Finish it from her chart so I can sign it."

## Task Files card (mount exactly these under /docs/filesystem, nothing else)
1. discharge_plan_draft_05232026.docx
...
Inspect the first trajectory's `find /docs` tree before using any scores. Require exactly the three task files above under `/docs/filesystem`, no `/docs/.apps_data`, and no stale filename (in particular no nephrology_consult_note_draft, the retired predecessor name). If the gate fails, delete every file from the Studio Task Files card and re-add only the current three.
