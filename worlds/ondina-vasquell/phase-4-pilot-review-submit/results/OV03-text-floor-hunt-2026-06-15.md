...
(1) chart-gap hunt = no-go; (2) cross-talk task-layer lever = bench-ceiling; (3) conflicting-authority = ceilings because this world's rebuttals are all loud (OV04/OV06 proven). This model on this clean 34-file world floors ONLY in two blind spots: COLD outside-knowledge (OV01) and COMPLETION-FRAME off-text synthesis (OV02). Both are used. A de-correlated third floor is not available here on current evidence. The mechanisms that would add one (cross-talk, conflicting-authority, buried rebuttal) require a substrate where the index data is NOT cleanly available - i.e., the next world built conflict-first.
