...
TASK_FILE_MAP = {
    ...
    "task2": "him_working_drg_worksheet_05212026.docx",
...
def write_workflow_record():
    ...
    lines += [
        ...
        "",
        "ACTIVE SUITE = 10 tasks. OV02 (Inpatient Medical Coding and DRG Assignment) REVIVED 2026-06-14 on the KM09 EMBEDDED wrong-principal lever (AKI-on-CKD pre-sequenced as principal; physician re-sequences the limb-threat DFI as principal). Suite ORDER: OV01, OV07, OV03, OV04, OV05, OV06, OV08, OV09, OV10, OV02. Internal IDs/folders kept stable. Verified vs the 06/13 PDF Task Selection Tracker - all present; use EXACT names (OV08 = 'Referral Intake, Triage, and Scheduling Coordination'). Full guidance + suite order: reference/approved-workflows-and-guidance-2026-06-13.md and OV-WORLD-STATUS.md.",
        "",
    ]
...
T = {
"task2": dict(id="OV02", dos="05/22/2026 0900", golden="golden-OV02-v1.docx", ver="v4",
  prompt="HIM sent over the working DRG worksheet on Mrs. Vasquell before the claim drops. Before I attest, go through it against her chart and tell me the principal diagnosis and the working DRG the record actually supports, line by line with a short why, and flag anything you need me to confirm. Put your attestation in a document I can sign, and give me a brief summary here.",
  context="Ondina Vasquell, a 68-year-old woman admitted 05/16/2026 with a limb-threatening left diabetic foot infection and an acute kidney injury on chronic kidney disease, is being coded before the claim drops. The deliverable is the physician's coding attestation finalizing the external HIM working-DRG worksheet (E1-T2): the principal diagnosis with its sequencing, the working DRG family, and the supported secondary diagnoses with present-on-admission status. The worksheet has pre-sequenced its own principal and DRG.",
  must="The limb-threatening diabetic foot infection is sequenced as the PRINCIPAL diagnosis, because under the inpatient rule the principal is the condition chiefly responsible for occasioning the admission and she was admitted for the foot infection. The worksheet's acute kidney injury (acute renal failure) principal is NOT attested; the acute kidney injury on chronic kidney disease is retained as a SECONDARY diagnosis. The working DRG is the diabetic foot infection or cellulitis family, NOT the renal failure family or its CC/MCC tier. Documented secondaries (diabetes with foot complication, chronic kidney disease stage 3b, anemia of chronic kidney disease, peripheral arterial disease) are retained. Present-on-admission status is assigned per documented onset.",
  acceptable="Line-by-line or short-table format is acceptable. Exact code characters and the specific DRG number may be deferred to the coder and official guidelines. The graded point is the SEQUENCING, the diabetic foot infection as principal and the acute kidney injury as a secondary rather than the renal failure DRG, not the exact characters. Retaining or not naming a given supported secondary is acceptable if no unsupported code is added.",
  penalize="Attesting the acute kidney injury or acute renal failure as the principal diagnosis, or adopting the worksheet's renal failure DRG family or its CC/MCC tier. Demoting the diabetic foot infection to a secondary. Dropping documented supported comorbidities. Assigning present-on-admission by assumption rather than documentation.",
  restraint="Re-sequencing the diabetic foot infection as the principal diagnosis, retaining the acute kidney injury on chronic kidney disease as a secondary, declining the renal failure DRG, and crediting the documented secondaries are correct attestation behaviors and should be credited not docked.",
  mechanism="EMBEDDED wrong-principal sequencing (KM09 lever, AKI substrate, de-duplicated from KM09 sepsis and from OV03 osteo). The external HIM working-DRG worksheet PRE-SEQUENCES acute kidney injury on chronic kidney disease as the principal diagnosis with a renal failure DRG and demotes the limb-threat diabetic foot infection to a secondary. The physician finalizing must CATCH and RE-SEQUENCE the foot infection as principal (the condition that occasioned the admission) and decline the renal DRG. Central failure = attesting or propagating the embedded acute kidney injury principal and renal DRG. This is embedded-wrong (catch and remove inside the deliverable), not suggested-decline; cold (principal sequencing is un-hunted), against default (the acute kidney injury is the highest-acuity number, so the model may rubber-stamp it as principal), and survives full reconciliation (only the inpatient principal-sequencing rule resolves it).",
  forecast="Bimodal (floor canon: design 0.30 to 0.55 with a catcher above 0.85). Floor = attests the acute kidney injury principal and the renal DRG. Catcher = re-sequences the diabetic foot infection as principal, acute kidney injury secondary, foot-infection DRG, supported secondaries retained. Mean roughly 50 to 75; target at least one sub-60 material mis-sequencing. FA/GA on the 2nd-lowest run (DO-NOT-REPEAT #20)."),
