...
"A scanned grayscale page from a hospital vascular laboratory study dated 05/19/2026, showing a lower-extremity arterial Doppler report. The page contains a synthetic EMR header for Ondina Vasquell matching the chart (Ordered by Lillian Everet, MD), a table of segmental pressures and indices for the left and right legs, and four stacked Doppler waveform tracings labeled by level (thigh, calf, ankle, toe). BOTH ankle-brachial indices read noncompressible (greater than 1.3) bilaterally, consistent with the report. Toe-brachial index right 0.84 and left 0.50; toe pressure right 92 mmHg and left 55 mmHg. Plain black-on-white medical form with thin grid lines, light scan noise and a faint photocopy gradient, sans-serif form text, no logos."

Hard constraints / negative prompt:
- Values must agree with EW9 EXACTLY: BOTH ankle-brachial indices noncompressible (greater than 1.3) bilaterally - do NOT print a measurable right-ankle index; the report says noncompressible on both sides. Toe-brachial index right 0.84 / left 0.50; toe pressure right 92 / left 55 mmHg. Ordering clinician is the attending of record, Lillian Everet, MD (verify the initial matches; an out-of-date initial like "M. Everet" is a fail). The toe pressure governs because the ankle is noncompressible - but the IMAGE only displays the numbers; it must not print an interpretation sentence that weights toe over ankle. Interpretation stays in the consult, not on the tracing.
...
- EW31 cross-check against EW9 by eye after ANY roster or value change: the ordering clinician initial matches the current attending (Lillian Everet, MD, not "M. Everet"); BOTH ankle indices read noncompressible; toe values right 92 / 0.84 and left 55 / 0.50. The byte gate cannot read image text, so this eye-check is the only catch. (2026-06-13: the first EW31 render failed both - stale "M. Everet" initial and a measurable right ankle - and must be regenerated to this spec.)
