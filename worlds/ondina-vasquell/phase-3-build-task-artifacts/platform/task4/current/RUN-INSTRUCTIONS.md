...
7. Run Task AutoQC (rerun only failing). Two expected residuals to DISPOSITION (not fix): (a) Self-Contained Guidelines - the grader needs the mounted chart to verify the over-closure; justify with the exact flagged text, do not flatten the grader. (b) Technical "agentic grader configured but enable_anthropic_api / model-access preflight is False" - recurring infra FALSE POSITIVE (KM02, KM07), not a writer-flippable flag. A short "tech issue" annotation is often accepted for this well-known infra flag (KM02 used exactly that, saved and verified); if it gets pushed back or fails QA Feedback (a bare one did in KM07), give the substantive, fact-referenced form: the agentic grader has model access at grading time and returns numeric scores on the trajectories (every OV04 pilot scored all ten, 0.10 to 0.85), so the warning reflects the linter's static configuration context, outside the task definition. Save and verify.
