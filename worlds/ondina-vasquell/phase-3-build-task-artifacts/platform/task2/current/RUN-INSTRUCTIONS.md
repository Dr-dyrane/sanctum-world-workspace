...
STATUS 2026-06-15: PATH A RE-PILOT DONE -> FAIR FLOOR (job d0795803, scores 0.08-0.12, mean 0.10, 10/10, vision-independent grader). BANKED PENDING REVIEW; OV's second floor. The agents missed the off-text synthesis 10/10 and the recut grader scored it fairly (credits a synthesis catch + inspect/hold hedge; requires no image). FA/GA at phase-4-pilot-review-submit/fa-ga/FA-GA-OV02-current.md; results at phase-4-pilot-review-submit/results/OV02-pathA-pilot-2026-06-15-job-d0795803.md. Do NOT re-pilot, re-tune, or make the intake signal louder (would convert the fair floor to a ceiling). The notes below are retained for upload mechanics only.

PRIOR STATUS (superseded): PATH A RECUT DONE - GO for re-pilot (one re-roll only). The image-miss v6 floored uniformly (job fe95a8a0, 0.08-0.15, 0 catchers) for the WRONG reason (bash-only harness, no vision either side; grader required "from the photograph"). RECUT to an off-text SYNTHESIS lever, vision-independent: the catch is now creditable from the text signals (a line-site photo was deliberately taken on transfer day + a new 38.0 temp vs the afebrile 05/21 baseline + IV abx running through that line -> suspect line infection -> inspect/hold/qualify transfer + remove/culture). Grader recut to credit the synthesis catch AND the correct inspect/hold hedge, and to cap only the affirmative dismissal ("no line complication documented") or transfer-as-is. Golden re-articulated to lead with the synthesis (photo = corroboration). Intake note UNCHANGED (pilot-calibrated: the v6 representative run had all three text signals and still wrote "no line complication documented"). Cold bench = FAIR FLOOR: under the real "finish the note" prompt both reviewers MISSED the line (matches the 10/10 pilot); the catch only surfaced on an explicit safety nudge. Verdict + bench: phase-4-pilot-review-submit/results/OV02-pathA-bench-2026-06-15.md; prereg OV02-pathA-pilot-preregistration.md.

CHANGE TO UPLOAD MECHANICS vs the original runbook below: the grader is now VISION-INDEPENDENT - you do NOT need to attach the image as a grader input, and the grader no longer aborts if the image is absent. The image stays in the task-file mount as corroboration (harmless if the harness cannot render it). Everything else (workflow string, prompt, the three task files, golden filename) is unchanged.

--- ORIGINAL RUNBOOK (pre-pilot, retained for upload mechanics only; ignore the "ATTACH the image to the grader" instruction - superseded above) ---
