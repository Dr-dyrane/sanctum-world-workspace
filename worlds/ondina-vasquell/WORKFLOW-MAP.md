...
State (2026-06-17): seven confirmed floors. OV01, OV02, OV04 Ready for Delivery; OV03 floored (FA/GA done); OV06 floored (FA/GA final, banking); OV07 floored (off-text image on Claims Denial, FA/GA done); OV08 floored (bimodal, mean 0.63, Utilization Review Concurrent Stay, FA/GA done, 5th lane). OV05 retired. TARGET RAISED (Dyrane 2026-06-17) to 8 to 10 shippable tasks across 6 to 7 distinct workflows: a workflow may carry 2 or more tasks, so reusing a strong lane to hit the count is acceptable, and Coding/DRG and CDI Query are back in play as candidate lanes. Six lanes in use; OV09 = Post-Acute Care Coordination Documentation (6th lane). v3 (embedded-wrong on the disposition headline) CEILINGED 0.84, range 0.72-0.92, no floor (job e9c38261), because disposition is the handoff headline and the model corrected it every run. v4 re-cut moves the embedded wrong to a background axis (three held oral agents resumed at discharge), pilot-ready; contrast v1 and osteo-image v2 retired. Dyrane tests by trajectory directly.
