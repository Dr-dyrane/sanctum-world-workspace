...
Grader: synthesis/from-chart deliverables need a chart-aware grader (chart-readable grading + a register note to verify doses/dates/labs/names against the provided chart before calling them invented); golden-only is safe only for a planted-artifact catch fully checkable against the golden. Grader filename reference must match the uploaded golden exactly. (Format: the KM five-block is canonical, matching KM's live-passing graders as of 2026-06-13: Preamble, Register Note, Section A Must be present and correct, Section B Acceptable variation ending with the verbatim two-failure-mode clause, Section C Patterns to reason about opening with the verbatim "These are patterns to reason about, not items to tick off." plus a correct-restraint credit. NO scoring bands. The v6.6 writer-doc's five-band scoring and A/B/C labels are stale versus what the live gate accepts; verify against KM's latest graders, not the writer-doc.)
