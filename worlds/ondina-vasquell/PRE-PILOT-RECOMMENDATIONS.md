...
6. **Match grader chart-access to mechanism; verify task-file fairness on built bytes.** Synthesis-from-chart -> chart-aware grader (include_input_files=true); planted-artifact catch -> golden-only. Confirm every task-level draft is a TRUE placeholder (asserts nothing about the scored item), exactly one visible task file per task, verified on extracted bytes - no leak via filename or duplicate mounted volume.
