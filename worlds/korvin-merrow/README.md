...
CURRENT STATUS (11 June 2026). `Healthcare_247_Merrow` is live (26 files). Of the ten tasks, six are delivered (KM01 through KM06) and four are under first human review (KM07 through KM10). KM07 is the fairness-corrected third version of the nephrology referral, which now has the model complete the medication-reconciliation status from the record and pilots in a fair mid-range; KM10, the documentation-integrity query response, pilots uniformly low and its reachability is not yet confirmed. The authoritative status is the platform board and the live records (`dashboard/km-world-dashboard.html`, `task-setup/KM-WORLD-PERFORMANCE-REPORT.md`, and each `task-setup/taskN/TASKN-STATE.md`). The dated paragraphs below are a frozen ~6/8 snapshot kept for provenance and are superseded by those sources.
...
KM04 post-fix rerun 6/8 late: rerun job `979dccde` is complete after the Sang grader/golden fix. Spread: 95, 90, 30, 88, 78, 88, 90, 35, 30, 40; mean 66.4; lowest score 0.30 shared by Attempts 3 and 9; Attempt 9 / run `976b2b18` selected for FA/GA. Current local FA/GA is re-derived at `task-setup/task4/fa-ga/FA-GA-current.md`; Alexander later completed platform FA/GA entry on Attempt 9; FA/GA AutoQC passed.
...
KM04 v2 update 6/8: v2 cleared the difficulty gate after job `709be0e8` (mean 0.689, three sub-70, tail 0.15) on the anemia-of-CKD / absent iron-workup propagation axis. Durable records: `task-setup/task4/runs/KM04-v2-taiga-results-709be0e8.md` and `task-setup/task4/runs/KM04-v2-grading-transcripts-709be0e8.md`; pre-Sang local FA/GA draft subject Attempt 5 is superseded; post-Sang platform FA/GA entry uses Attempt 9 (0.30) and FA/GA AutoQC passed. The v2 platform set remains under `task-setup/platform/task4/current/`; v1 evidence is archived at `task-setup/platform/task4/archive/2026-06-08-pre-clean/`. KM04 PL #1-#2 are drafted locally; no KM04 PL has been platform-submitted and no final review exists.
...
- `task-setup/task3/build-phase-drafts/KM03-golden-clinical-register-guide.md` (local golden-register support, not platform-current)
- `task-setup/task3/build-phase-drafts/grader-guidelines-task3-RESTRUCTURED.txt` (local FA/GA support helper, not platform-current)
