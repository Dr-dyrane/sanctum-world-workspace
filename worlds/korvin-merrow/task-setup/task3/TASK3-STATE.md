...
Status (6/8, current): KM03 AT TAIGA TRAJECTORY RERUN (post grader/golden fix). Sang (Trigeminus pod) review fixes are applied and the grader was confirmed to carry the trap (not formatting-only) and approved in structure: grader restructured into the five-block format (Preamble/Register Note/Section A/B/C, golden named verbatim, two-failure-mode clause and Section C opener verbatim); golden elevated to the pipeline clinical register (named consultants/staff/agencies, concrete specifics; CPAP trap-carrier line kept plain, fingerprint clean); GA mapped to Sections A/B/C; FA unchanged (Sang approved). Active set under `../platform/task3/current/` with old grader+golden archived under `../platform/task3/archive/2026-06-08-pre-restructure/`. NEXT GATE: re-run the Taiga trajectories against the new grader+golden and confirm the spread holds. KIV AHEAD: after the rerun, re-derive BOTH FA + GA on the new lowest run (current `fa-ga/FA-GA-current.md` is keyed to the pre-fix job `877aa204` Attempt 9 / 0.20; treat as provisional until the rerun lands). PL prep stays on hold until FA/GA is settled. The difficulty-clear history below is preserved as the difficulty record.
