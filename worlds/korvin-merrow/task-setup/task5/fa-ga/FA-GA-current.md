...
The deduction falls on the central Section C pattern, the premature cardiorenal restart. The run resumed sacubitril/valsartan and furosemide on the unverified home-BP readings, against the chart's staged-restart-deferred plan. Placing it near the floor is proportionate and symmetric, since the run that kept all four agents held and deferred the restart to cardiology and nephrology scored 0.95, so the deduction tracks the restart the chart forbids and not the restraint Section A credits.

## Carried forward (NOT field content - for our learnings)
Abi fairness fix validated on the re-pilot: re-attributing the home BP to the patient (interval + item 1) kept the task bimodal and fair while lifting the mean from 0.36 to ~0.50. Floor signature (post-fix): resume held GDMT on the patient-reported home BP; the post-fix floors no longer rely on an immunizations-current slip (Att1 reviewed immunizations correctly), so the scored axis is cleanly the restart. Catch signature (Att2, 0.95): held all four agents, treated the BP as patient report, deferred to cardiology/nephrology.
