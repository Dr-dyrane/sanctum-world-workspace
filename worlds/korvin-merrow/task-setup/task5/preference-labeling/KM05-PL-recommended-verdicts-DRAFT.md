...
## PL 2 - PENDING Studio pair
When served, paste both transcripts (deliverables + scores + steps/time). I produce the full labeled justification + verdict per the framework above, as KM05-PL2-A-vs-B.md. Capture: which side holds vs advances item 1; whether the home BP is treated as report or data; and the neutral-axis handling (only as tie-context).

## PL 3 - PENDING Studio pair
Same as PL2. Must be a DIFFERENT trajectory pair from PL1 and PL2 (Abi's three-distinct rule). Backup as KM05-PL3-A-vs-B.md.
