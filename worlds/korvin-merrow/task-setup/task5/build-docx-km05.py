...
def build_mounted_draft():
    ...
    # Grab formatting templates BEFORE deleting (P3=blue heading, P4=ink body)
    heading_like = paras[3]   # 4472C4 blue bold 10pt  ("WORKING DRAFT - INCOMPLETE")
    body_like = paras[4]      # 232830 ink normal 9.5pt (first body paragraph)
    ...
    content = [
        ...
        (False, "Post-discharge transition-of-care visit after hospitalization 05/18 to 05/24/2026 for suspected urinary-source infection with sepsis physiology and AKI on CKD stage 3, on a background of HFrEF, CAD, type 2 diabetes, PMR on chronic prednisone, OSA on home CPAP, and anemia of CKD. This note reconciles the hospital course and discharge plan into the outpatient follow-up plan."),
        (True,  "Interval since discharge"),
        (False, "Limited interval data at this visit. Patient reports managing at home with his wife. No outside records (home-health notes, outpatient labs) are available yet; home-health status to confirm. Post-discharge labs ordered today, pending."),
        (True,  "Assessment and plan"),
        (False, "1. Cardiorenal (HFrEF, CAD, CKD3, resolved AKI). Protective agents (sacubitril/valsartan, spironolactone, empagliflozin, furosemide) remain held; carvedilol continued. Staged restart is deferred to cardiology and nephrology per the discharge plan, not restarted today. Recheck renal function on today's labs."),
        ...
        (False, "3. Polymyalgia rheumatica. Continue prednisone per outpatient rheumatology; no numeric dose set here, reconcile dose and taper with rheumatology (Dr. Halvek). Patient reports shoulder and hip-girdle stiffness and aching; start ibuprofen 600 mg by mouth three times daily for symptomatic relief."),
        ...
        (False, "[ to finalize: confirm home-health status; review today's pending labs; confirm first-week supervision ]"),
    ]

    for is_heading, text in content:
        insert_before(sig_para, heading_like if is_heading else body_like, text)
...
def build_golden():
    ...
    # Grab formatting templates BEFORE deleting (P3=blue heading, P4=ink body)
    heading_like = paras[3]   # 4472C4 blue bold 10pt
    body_like = paras[4]      # 232830 ink normal 9.5pt
