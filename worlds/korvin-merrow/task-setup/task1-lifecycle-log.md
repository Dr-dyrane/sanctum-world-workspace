...
- Task 1: Abi first human review returned SEND BACK / rework required. Do not enter Tasks 2-6 until Task 1 rework is accepted or Alexander explicitly authorizes a bounded parallel prep.
- Immediate Task 1 rework path: delete both task files in RLS, replace prompt with `platform/task1/prompt-task1-v2.txt`, replace golden with `platform/task1/golden-response-task1-v3.docx`, replace grader guidelines with `platform/task1/grader-guidelines-task1-v5.txt`, rerun Task AutoQC, rerun Taiga trajectories, then rewrite FA/GA on the NEW single lowest run using Abi's compact format.
