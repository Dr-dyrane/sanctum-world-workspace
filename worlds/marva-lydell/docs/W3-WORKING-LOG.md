...
## 2026-06-20 - W3 Spec AutoQC round 2: Task 8 workflow label
Rerun flagged Task Portfolio / Workflow Labels: Task 8 label "Corrective Action Plan (CAP) Development and Tracking" did not match the header tracker "Root Cause Analysis". The approved categories (Sanctum Task Selection Categories 06/19) name the patient-safety lane "Patient Safety Event Investigation and Root Cause Analysis", so the header "Root Cause Analysis" was correct and Task 8's CAP label was the outlier. Moved Task 8 onto the RCA lane: title -> "Post-Readmission Safety Event Root Cause Review", workflow -> "Patient Safety Event Investigation and Root Cause Analysis", deliverable and format -> root cause analysis report, with corrective actions kept as the RCA's recommendations. Rebuilt, gate clean (fingerprint matches base), zero dashes, priorities 7 P0 / 2 P1 / 1 P2. Commit 74ba50d. Writer re-uploads and uses Rerun 1 failing.
