...
## Run binding
Bind the FA to ONE run: the SECOND-lowest distinct score in the current valid run set (King P, 2026-06-14; supersedes the older "lowest run"). A single anomalous floor should not drive the analysis. If Taiga trajectories and QA are rerun, the prior FA/GA is stale. Redo FA/GA from the latest run set. If the strict second-lowest distinct has no full transcript, bind to the nearest run you do have and say so. The Status line is internal and is NOT pasted into Studio, so the submitted FA field must itself name the run: open the FA's first paragraph with "On trajectory N" (the 1-10 count, the same number as the attempt). Use that one identifier, not both attempt and trajectory, and not the run or trajectory hash; the hashes and the distribution stay in the Status line. The GA names the same run the same way ("The grader scored trajectory N at X"). Lead the FA with the Output Score as the LITERAL first element (updated 2026-06-20: context before the score, including the run name, is the recurring AutoQC fail). The FA first words are the writer-score line "Overall Failure Score: X.XX / 1.0", then it names the run ("On trajectory N") and gives a brief failure-framed justification, before the flaw analysis.

## Failure Analysis - what the agent got wrong and why
Cover these in prose. The OUTPUT SCORE now LEADS (updated FA template, 2026-06-20: state your score first):
1. The OUTPUT SCORE, literally first. Start the FA with the score line, either the 0-1 form `Overall Failure Score: X.XX / 1.0` or the Studio-confirmed percent form `Overall Score: NN%`, then name the run ("On trajectory N") and give a brief justification. The score is the first substantive element, before any context including the run name (2026-06-20 reviewer rule, the recurring fail). The justification is failure-framed (why it scores this low), never a competent baseline or anything the agent did right; the live AutoQC flags any positive framing, even one line or a "to its credit" clause. The number is Alexander's own score, at or below 0.70 (above 70 means too easy, do not proceed), not the trajectory score; the distribution stays in the Status line.
2. What the agent got WRONG, in order of materiality. Lead with the most consequential flaw, then the other notable flaws. Anchor each flaw to what the golden does instead: for everything the output got wrong, say what the right answer looked like. The central failure is a turn, the agent had the contradicting evidence and failed anyway; naming the evidence it held is allowed only as the setup for the failure (it had X in hand and still did Y), never as credit.
3. Why it MATTERS - the clinical mechanism and the consequence. Quantify when you can. State the severity tier (a patient-safety failure is not a documentation preference). State plainly that the output would not be deliverable by any competent practicing clinician, and what a competent clinician would have done differently; if you cannot say it is not deliverable, the failure is insufficient (Phase 3).
...
WRITER'S SCORE (project guidance, 2026-06-18). Verbatim project guidance: "New Failure Analysis guidance. Please submit a score with your failure analysis about how you think the model did in your task. This score should be your own score, not the trajectory score, and will be compared against the agentic grader." Separately from the grader rating, submit your OWN score of how the model did on the task, alongside the FA: your independent 0.0 to 1.0 assessment of the model's performance, NOT the trajectory score, compared against the agentic grader. Score on the task's own logic (central failure caps low, partial credit for accurate items, no credit for refusing). It usually lands near the grader; when it diverges, say why in the GA's calibration move. The writer's score goes in the Studio score field AND inside the submitted FA field, because the Status line is internal and is not pasted into Studio. The FA must LEAD with the exact required line `Overall Failure Score: X.XX / 1.0` (the Output Score, stated first per the 2026-06-20 FA template), with a brief justification. Record it in the Status line too. It is the writer's own number, distinct from the banned grader-rating line (Poor / Fair / Good / Great), which never appears in the file, not even the Status line.
...
Headings as prose beats (decision B). FA: the Output Score first (the writer's `Overall Failure Score: X.XX / 1.0` line and a brief failure-framed justification), then What the Agent Got Wrong (most consequential flaw first, anchored to the golden), then Why the Failure Matters (would not be deliverable by a competent clinician). GA: Suggested Score (restate the writer's score and name the gap to the grader's score) and Score Comparison (the grader audit, what it got right and wrong, citing the grading summary directly). Do not reference the self-score bands in the prose.
...
- Both share the Alexander discipline: varied, declarative sentences; no AI transitions (Furthermore, Moreover, Consequently, Notably, In addition); no scaffolding (OCR, directory listings, "the transcript shows", tool calls); name the clinical anchors.
...
- Self-check before handing back: run `python3 tools/verify/lint_fa_ga.py <the FA/GA file>` and clear every FAIL. It enforces this skill mechanically (the FA leads with the `Overall Failure Score` line then names the run, single run only, `Overall Failure Score` line, a dated source-document citation, two paragraphs, under 1000 chars, no dashes, no rating word, no banned phrases) and warns on semicolons and sentences over 24 words. Fix until it prints "clean".
...
FA: leads with the Output Score line `Overall Failure Score: X.XX / 1.0` as the first element, then names the run "On trajectory N", with a brief failure-framed justification (the score is literally first, 2026-06-20); NO competent baseline or praise anywhere (any positive framing fails AutoQC); then the most consequential flaw first, anchored to the golden; the clinical mechanism, consequence, and severity tier; states plainly it would not be deliverable by a competent clinician and what one would have done; names the specific source documents by author or date (not a generic reference); references only this run, never other runs' scores; failure-only.
