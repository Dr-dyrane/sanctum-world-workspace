...
12. `worlds/korvin-merrow/task-setup/platform/task3/current/` for the KM03 active platform set after Codex gate, upload, and qcaud_6b pass.
...
15. `worlds/korvin-merrow/task-setup/platform/task1/` for live Task 1 platform artifacts.
16. `worlds/korvin-merrow/task-setup/platform/task1/archive/` only for historical Task 1 iterations.
...
Status: Task 1 final human review complete / approved. Task 1 AO rework, hardening, Abi pre-check, revised platform entry, pilot runs, FA/GA, Preference Labeling, and final review are complete. Task 2 / KM02 is complete through writer workflow and Awaiting Final Review after the reseed/date fix; Task AutoQC / Taiga gates passed qcaud_5e, qcaud_4a, and qcaud_ef, final FA/GA uses Attempt 8 at 0.30 from v3 job `8f393839`, and submitted PL evidence lives under `task2/preference-labeling/`. KM03 platform files were uploaded under Alexander operation; Task AutoQC passed 36/36 (`qcaud_6b`) after DOCX core-metadata scrub; Taiga Trajectories & QA are running. Further platform actions remain under Alexander's direct operation. Tasks 4-6 remain held unless Alexander explicitly authorizes bounded parallel prep or the next pod-directed task step.
Current contents:
...
- `worlds/korvin-merrow/task-setup/task3/TASK3-STATE.md`: Task 3 current-state cockpit. KM03 local Codex gate passed on 2026-06-07; platform files were uploaded under Alexander operation; Task AutoQC passed 36/36 (`qcaud_6b`); Taiga Trajectories & QA are running. Open: Alexander physician sign-off on golden clinical content.
- `worlds/korvin-merrow/task-setup/task3/KM03-state-log.md`: Task 3 live event log for build, AutoQC, and run-state changes. Current event: Mode A build scrubbed, Codex GO, qcaud_6b pass, trajectories/QA running.
