...
Current status: Packaging Construction / CANDIDATE REVIEW. Locked and complete layers include World Spec v1, File Inventory Architecture v1, File Inventory v1, World-Level Synthetic File Layer FI-W01 through FI-W22, Task-Level Context Files FI-T01 through FI-T07, Supplementary Files FI-S01 through FI-S04, the Entire File Ecosystem, Task Prompt Architecture v1, Task Prompts TP-KM01 through TP-KM06, Expected Output Architecture v1, Expected Outputs EO-KM01 through EO-KM06, Golden Architecture v1, Golden-KM01 through Golden-KM06, Grader Guidance Architecture v1, GG-KM01 through GG-KM06, AutoQC Architecture v1, AutoQC Construction v1, and Packaging Architecture v1. AutoQC Architecture v1 locked artifacts are under `worlds/korvin-merrow/autoqc-architecture/locked/`: `autoqc-architecture-v1.md` and `autoqc-architecture-validation-review.md`, with ratification at `worlds/korvin-merrow/autoqc-architecture/ratifications/autoqc-architecture-ratification.md`. AutoQC Construction locked artifacts are under `worlds/korvin-merrow/autoqc/locked/`: `autoqc-construction-v1.md` and `autoqc-construction-validation-review.md`, with ratification at `worlds/korvin-merrow/autoqc/ratifications/autoqc-construction-ratification.md`. AutoQC is complete. Packaging Architecture locked artifacts are under `worlds/korvin-merrow/packaging-architecture/locked/`: `packaging-architecture-v1.md` and `packaging-architecture-validation-review.md`, with ratification at `worlds/korvin-merrow/packaging-architecture/ratifications/packaging-architecture-ratification.md`. Packaging Architecture is complete. Packaging Construction candidate artifacts are under `worlds/korvin-merrow/packaging/candidate-review/`: `packaging-construction-v1.md` and `packaging-construction-validation-review.md`. Next eligible phase is Packaging Construction Review. AutoQC runs, AutoQC responses, scoring rubrics, scoring thresholds, pass/fail bands, point allocations, DOCX submission packaging, manifests, final submission packages, submission artifacts, and RL Studio upload have not started.
...
107. `worlds/korvin-merrow/packaging/candidate-review/packaging-construction-v1.md` for Packaging Construction v1 candidate package.
108. `worlds/korvin-merrow/packaging/candidate-review/packaging-construction-validation-review.md` for Packaging Construction validation review.
109. `worlds/korvin-merrow/world-spec-prep/planning-scaffolds/task-architecture-interview-v1.md` for historical interview basis.
110. `worlds/korvin-merrow/world-spec-prep/decision-logs/physician-decision-log-02.md` for ratified Clinical Story Skeleton decisions and construction-lock status.
111. `reference/world-spec-guidelines/08_autoqc_master_index.md` only when exact AutoQC checks are needed.
...
- `candidate-review/` is reserved for active candidate artifacts only. Inactive candidate-review folders should be absent from the visible tree and recreated on demand when a new candidate artifact exists. The current active candidate-review folder is `worlds/korvin-merrow/packaging/candidate-review/`.
...
- `candidate-review/`: active Packaging Construction v1 candidate artifacts.
- `locked/`: future locked Packaging Construction artifacts after ratification.
- `ratifications/`: future ratification records for Packaging Construction.
...
| File | Role | Duplication note |
| --- | --- | --- |
| `worlds/korvin-merrow/packaging/candidate-review/packaging-construction-v1.md` | Candidate Packaging Construction package. | Maps PKG-KM01 through PKG-KM08, execution order, dependency order, blocked dependencies, required inputs, future output targets, transcript/DOCX/manifest/upload/reconciliation workflows, authorization gates, and stop points. It does not create packaging artifacts. |
| `worlds/korvin-merrow/packaging/candidate-review/packaging-construction-validation-review.md` | Candidate validation review for Packaging Construction v1. | Verifies alignment with locked Packaging Architecture, locked sources, submission-reference materials, transcript boundary, DOCX boundary, manifest boundary, upload boundary, AutoQC boundary, and locked-artifact protection. |
