...
KM03 v2.2 platform update 6/8: active pre-Taiga platform files are `worlds/korvin-merrow/task-setup/platform/task3/current/prompt-task3-v2.2.txt`, `worlds/korvin-merrow/task-setup/platform/task3/current/discharge_planning_summary_draft_05242026.docx`, `worlds/korvin-merrow/task-setup/platform/task3/current/golden-KM03-v2.2.docx`, and `worlds/korvin-merrow/task-setup/platform/task3/current/grader-guidelines-task3-v2.2.txt`. Task AutoQC passed 36/36 (`qcaud_fc`). Taiga trajectories are intentionally held; no v2.2 Taiga run, FA/GA, PL, final review, additional upload, or AutoQC response exists. The older v2.1 files remain in `platform/task3/current/` as retained evidence until a path-hygiene move is explicitly authorized; for active KM03 work use the v2.2 filenames.
...
13. `worlds/korvin-merrow/task-setup/platform/task3/current/` for the active KM03 v2.2 pre-Taiga set (`prompt-task3-v2.2.txt`, `discharge_planning_summary_draft_05242026.docx`, `golden-KM03-v2.2.docx`, `grader-guidelines-task3-v2.2.txt`). V2.1 evidence lives under `platform/task3/archive/v2.1-difficulty-failed-after-58b5f3e3/`; Taiga is held.
14. `worlds/korvin-merrow/task-setup/task3/KM03-3rd-reader-review-and-build-gates.md` plus `task3/build-phase-drafts/01-build-phase-review-responses.md` for the gate history and completion-frame draft response.
15. `worlds/korvin-merrow/task-setup/task3/build-phase-drafts/KM03-v2.2-KM02-BAR-PLAN.md` for the current primary KM03 v2.2 mechanism source. It defines the tuned OSA/CPAP fabricated-objective-result mechanism used by the active v2.2 platform set.
16. `worlds/korvin-merrow/task-setup/task3/build-phase-drafts/KM03-v2.2-KM02bar-review-request-for-claude-ai.md` for the self-contained Claude.ai review request paired to the KM02-bar plan. Its review outcome is folded into the active v2.2 platform set.
17. `worlds/korvin-merrow/task-setup/task3/build-phase-drafts/KM03-v2.2-FINAL-PLAN.md` for the superseded Lenora supervision-fact review packet. Review history only unless Alexander re-selects it.
18. `worlds/korvin-merrow/task-setup/task3/build-phase-drafts/KM03-v2.2-reconciliation-6-8.md` for the superseded Lenora decision note plus no-moderate correction. Review history only unless Alexander re-selects that mechanism.
19. `worlds/korvin-merrow/task-setup/task3/build-phase-drafts/` for draft G1-G3 closure review material and superseded v2.2 planning only; not platform-current artifacts.
20. `worlds/korvin-merrow/task-setup/task4/TASK4-STATE.md` for KM04 v1 difficulty-failed state, boundaries, and next gates.
21. `worlds/korvin-merrow/task-setup/task4/runs/KM04-taiga-results-55ee209f.md` for the 6/8 trajectory result showing KM04 v1 is too easy.
22. `worlds/korvin-merrow/task-setup/platform/task4/current/` for the KM04 v1 platform set as evidence only. It is not a shipping candidate after the 6/8 difficulty failure unless Alexander explicitly overrides.
23. `worlds/korvin-merrow/task-setup/task4/build-phase-drafts/KM04-codex-black-team-review-6-7.md` for the Codex black-team verdict before trajectory evidence superseded the difficulty prediction.
24. `worlds/korvin-merrow/task-setup/task4/design/KM04-design-plan-for-review.md` plus `task4/KM04-prebuild-review-and-build-gates.md` for KM04 mechanism history.
25. `worlds/korvin-merrow/task-setup/task4/build-phase-drafts/` for KM04 draft review material, Claude.ai review, Codex review, and copied inputs only.
26. `worlds/korvin-merrow/task-setup/task5/TASK5-STATE.md` for KM05 review-only state, source chain, boundaries, and next gates.
27. `worlds/korvin-merrow/task-setup/task5/design/KM05-design-plan-for-review.md` plus `task5/KM05-prebuild-review-and-build-gates.md` for KM05 mechanism history and gate checklist.
28. `worlds/korvin-merrow/task-setup/task5/design/KM05-claude-ai-proposal-6-7.md` and `worlds/korvin-merrow/task-setup/task5/design/KM05-codex-black-team-6-7.md` for KM05 review convergence, hold recommendation, and clean-baseline-versus-escalation decision surface.
29. `worlds/korvin-merrow/task-setup/task5/build-phase-drafts/` for KM05 copied inputs, byte/source audit, draft prompt, golden/grader deltas, mounted-source draft, manifest plan, pilot preregistration, and Claude review request only.
30. `worlds/korvin-merrow/task-setup/platform/task1/` for live Task 1 platform artifacts.
27. `worlds/korvin-merrow/task-setup/platform/task1/archive/` only for historical Task 1 iterations.
...
- `worlds/korvin-merrow/task-setup/task3/TASK3-STATE.md`: Task 3 current-state cockpit. KM03 v2.1 is difficulty-failed; active v2.2 is pre-Taiga hold after Task AutoQC PASS (`qcaud_fc`).
- `worlds/korvin-merrow/task-setup/task3/KM03-state-log.md`: Task 3 live event log for build, AutoQC, and run-state changes. Current event: v2.2 platform set is active/current, AutoQC passed, and Taiga is held.
...
- `worlds/korvin-merrow/task-setup/platform/task3/current/`: KM03 platform-current folder. Active v2.2 pre-Taiga set is `prompt-task3-v2.2.txt`, `discharge_planning_summary_draft_05242026.docx`, `golden-KM03-v2.2.docx`, and `grader-guidelines-task3-v2.2.txt`; Task AutoQC passed 36/36 (`qcaud_fc`) and Taiga is held. Do not proceed to Taiga, FA/GA, PL, final review, rerun AutoQC, rerun agents, rerun QA, upload additional files, or mutate RL Studio without exact Alexander authorization.
...
- Task 1 final human review is complete. Task 2 / KM02 is COMPLETE / RFD (Ready for Delivery) after passing qcaud_5e, qcaud_4a, qcaud_ef, Preference Labeling with submitted verdict B / B++, and Janette's 6/8 final review; KM03 escalation through v2.1 failed the difficulty gate after job `58b5f3e3`; KM04 v1 failed the 6/8 trajectory difficulty gate; KM05 is review-converged and not build-ready. Do not run additional task uploads, additional agent runs, additional QA runs, create AutoQC responses, create scoring artifacts, resubmit preference labels, alter KM04 platform-current files, redesign KM04, build or stage KM05 platform-current files, or mutate RL Studio beyond the exact authorized step without explicit Alexander authorization.
