...
Status: active. Task 1 AO Review Rework is active after Abi send-back; RLS rework entry and Task AutoQC rerun are complete at 2/68 justified. Tasks 2-6 remain held until Task 1 rework is accepted or Alexander explicitly authorizes bounded parallel prep.
Current contents:
- `worlds/korvin-merrow/task-setup/task1-lifecycle-log.md`: canonical Task 1 build/run source of truth. It records pod assignment, task setup details, Task AutoQC remediation, trajectory run state, FA/GA AutoQC, Abi send-back, drafted rework artifacts, and next action sequence.
...
- `worlds/korvin-merrow/task-setup/platform/task1/`: Task 1 platform provenance folder. Contains uploaded/superseded task files, golden-response v1/v2/v3 DOCX files, grader-guideline v1-v5 TXT files, and prompt/task memo variants. `prompt-task1-v2.txt`, `golden-response-task1-v3.docx`, and `grader-guidelines-task1-v5.txt` are the current post-Abi local rework drafts. v4 grader guidance is superseded for platform-facing rework. These are platform-facing Task 1 copies, not replacements for the locked canon chain unless explicitly reconciled.
...
- `worlds/korvin-merrow/task-setup/task1/FA-GA-final.md`: final submitted Task 1 Failure Analysis and Grader Analysis wording. Platform status: saved on batch v2, run `aef58074`; GA rated Great; FA/GA AutoQC submitted at 1/14 with the single false positive justified.
Boundaries:
...
- Do not start preference labeling, additional task uploads, additional agent runs outside batch v3, additional QA runs outside batch v3, create AutoQC responses, create scoring artifacts, or mutate RL Studio from this folder without explicit authorization. Current platform state is Task 1 AO Review Rework after Abi send-back; next is batch v3 Taiga trajectories/QA.
