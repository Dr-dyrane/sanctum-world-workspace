...
Status: active. Task 1 is In First Human Review, picked up by Abimbola O / Abi; Tasks 2-6 remain held until feedback.
Current contents:
- `worlds/korvin-merrow/task-setup/task1-lifecycle-log.md`: canonical Task 1 build/run source of truth. It records pod assignment, task setup details, Task AutoQC remediation, trajectory run state, FA/GA AutoQC, First Human Review pickup by Abi, and next action sequence.
...
- `worlds/korvin-merrow/task-setup/platform/task1/`: Task 1 platform provenance folder. Contains the uploaded task files, golden-response v1/v2 DOCX files, grader-guideline v1-v4 TXT files, and the revised discharge medication reconciliation request memo. `grader-guidelines-task1-v4.txt` is the current `/docs`-aware template; v3 is superseded. These are platform-facing Task 1 copies, not replacements for the locked canon chain unless explicitly reconciled.
