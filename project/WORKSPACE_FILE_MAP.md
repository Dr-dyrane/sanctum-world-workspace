...
- `worlds/korvin-merrow/task-setup/platform/task1/`: Task 1 platform provenance folder. Contains the uploaded task files, golden-response v1/v2 DOCX files, and grader-guideline v1-v3 TXT files. These are platform-facing Task 1 copies, not replacements for the locked canon chain unless explicitly reconciled.
...
- Do not start Failure Analysis / Grader Analysis, preference labeling, additional task uploads, additional agent runs, additional QA runs, create AutoQC responses, create scoring artifacts, or mutate RL Studio from this folder without explicit authorization.
