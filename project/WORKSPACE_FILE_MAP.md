...
Current status: World Spec Construction Preparation / Clinical Story Timeline Package v1 locked. Identity Package review addendum recorded as carry-forward notes only. World Spec drafting has not started.
...
15. `worlds/korvin-merrow/world-spec-prep/decision-logs/physician-decision-log-02.md` for ratified Clinical Story Skeleton decisions.
16. `reference/world-spec-guidelines/08_autoqc_master_index.md` only when exact AutoQC checks are needed.
...
- `candidate-review/`: artifacts awaiting physician decision or lock. Currently empty after Clinical Story Timeline Package v1 ratification.
