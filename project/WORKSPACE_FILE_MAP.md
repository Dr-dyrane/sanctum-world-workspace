...
117. `korvin-merrow-final-submission-staging/` for generated submission-facing artifact staging. Contains the final World Spec DOCX, FI-W/FI-S DOCX exports, Claude transcript DOCX/PDF, optional Brainstorm copy, and empty hold-not-upload folder. This is staging only; it is not a manifest, zip, upload, submission, AutoQC response, or scoring artifact.
118. `worlds/korvin-merrow/world-spec-prep/planning-scaffolds/task-architecture-interview-v1.md` for historical interview basis.
119. `worlds/korvin-merrow/world-spec-prep/decision-logs/physician-decision-log-02.md` for ratified Clinical Story Skeleton decisions and construction-lock status.
120. `reference/world-spec-guidelines/08_autoqc_master_index.md` only when exact AutoQC checks are needed.
