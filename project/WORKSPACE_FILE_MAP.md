...
3. `worlds/korvin-merrow/task-setup/platform/task2/README.md` for the current Task 2 clean-pilot platform set and escalation hold rule.
4. `worlds/korvin-merrow/task-setup/task2/KM02-design-plan-for-review.md` for Task 2 red-team study and discriminator history.
5. `worlds/korvin-merrow/task-setup/platform/task1/` for live Task 1 platform artifacts.
6. `worlds/korvin-merrow/task-setup/platform/task1/archive/` only for historical Task 1 iterations.
...
- `worlds/korvin-merrow/task-setup/platform/task2/`: Task 2 / KM02 platform-ready clean-pilot set. Contains `README.md`, `prompt-task2.txt`, `golden-KM02-v5.docx`, `grader-guidelines-task2.txt`, and `hold/discharge_summary_draft_incomplete.md`. Clean pilot uses no mounted task file. The hold draft is escalation-only and should be mounted only if the clean pilot is too easy. Task AutoQC passed after stripping the golden-only synthetic footer token; the finalized world files were not changed.
...
- Task 1 final human review is complete. Task 2 clean-pilot materials are staged locally; do not run additional task uploads, additional agent runs, additional QA runs, create AutoQC responses, create scoring artifacts, or mutate RL Studio without explicit Alexander authorization for the exact step.
