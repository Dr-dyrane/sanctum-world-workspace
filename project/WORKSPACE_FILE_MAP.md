...
3. `worlds/korvin-merrow/task-setup/task2/KM02-escalation-results.md` for the current Task 2 escalation spread and fair-discriminator summary.
4. `worlds/korvin-merrow/task-setup/task2/KM02-taiga-qa-log.md` for the current QA gate, including the exact `tech issue` annotation rule.
5. `worlds/korvin-merrow/task-setup/task2/FA-GA-current.md` for the prepared run-7 FA/GA candidate after QA clears.
6. `worlds/korvin-merrow/task-setup/task2/KM02-pilot-failure-analysis.md` for why the clean pilot failed as a difficulty mechanism.
7. `worlds/korvin-merrow/task-setup/platform/task2/README.md` and `platform/task2/escalation/README.md` for platform artifact provenance.
8. `worlds/korvin-merrow/task-setup/task2/KM02-design-plan-for-review.md` for Task 2 red-team study and discriminator history.
...
Status: Task 1 final human review complete / approved. Task 1 AO rework, hardening, Abi pre-check, revised platform entry, pilot runs, FA/GA, Preference Labeling, and final review are complete. Task 2 / KM02 clean pilot completed and was too easy; the colleague-draft escalation run produced a usable failure spread and the current gate is Taiga QA Feedback AutoQC / QA handling, then FA/GA if cleared. Further platform actions remain under Alexander's direct operation. Tasks 3-6 remain held unless Alexander explicitly authorizes bounded parallel prep or the next pod-directed task step.
Current contents:
...
- `worlds/korvin-merrow/task-setup/task2/KM02-design-plan-for-review.md`: Task 2 / KM02 pre-build discriminator design plan for external red-team review. Contains the correction that agent-read DOCX files, not markdown or reference copies, govern culture/antibiotic facts. Study/review material only; it does not create a platform task, task upload, agent run, QA action, or platform mutation.
- `worlds/korvin-merrow/task-setup/task2/RED-TEAM-BRIEF-KM02.md`: paste-ready KM02 red-team brief for claude.ai review. Directs reviewers to the agent-read world layer and names the v5 golden, prompt draft, grader, colleague-draft escalation, canon folder, continuity folder, and bundle contents.
- `worlds/korvin-merrow/task-setup/task2/prompt-task2-v1-DRAFT.txt`: Task 2 prompt draft for physician edit/ownership and red-team review. Draft only; not uploaded and not platform-authoritative.
- `worlds/korvin-merrow/task-setup/task2/Golden-KM02-v2-DRAFT.md`: historical/rationale markdown for the KM02 golden rebuild and correction trail. Superseded where it conflicts with the v5 source/DOCX correction.
- `worlds/korvin-merrow/task-setup/task2/golden-KM02-source.md`: source markdown for generating the KM02 v5 golden DOCX through `tools/generate_reference_files.py`; current source includes restored identity band coverage and the comorbidity ceiling fix.
- `worlds/korvin-merrow/task-setup/task2/golden-KM02-v5.docx`: current KM02 red-team golden candidate. Generated from `golden-KM02-source.md` through the Epic-style world-file builder and banner-stripped to match the agent-read uploaded world chrome. Codex gate passed identity band, comorbidity, culture/antibiotic, integrity, and render checks. Candidate review/platform-prep artifact only.
- `worlds/korvin-merrow/task-setup/task2/grader-guidance-KM02-v1.md`: task-specific KM02 grader draft derived from locked GG-KM02 with culture/antibiotic scope corrections. Review/prep artifact only; platform-ready meta-stripped text lives in `platform/task2/grader-guidelines-task2.txt`.
- `worlds/korvin-merrow/task-setup/task2/colleague-draft-KM02.md`: KM02 escalation design note and rationale. It is not mount-safe as a whole file; only the clean clinical draft extracted to `platform/task2/hold/discharge_summary_draft_incomplete.md` may be used if the clean pilot needs escalation.
- `worlds/korvin-merrow/task-setup/task2/KM02-redteam-bundle.zip`: ignored local KM02 red-team bundle containing the design plan, prompt draft, v5 golden/source, grader draft, colleague-draft escalation, locked KM02 canon, continuity references, and the agent-read 26-file world snapshot for red-team review. Review convenience bundle only; not repository canon, not a submission package, and not an upload artifact. Supersedes `KM02-review-bundle.zip`.
- `worlds/korvin-merrow/task-setup/task2/KM02-pilot-failure-analysis.md`: analysis of the first clean KM02 trial. Records 10 trajectories scoring 92-97, no true penalties, no fabrication, and the conclusion that clean discharge-summary synthesis is too easy on this disciplined chart.
- `worlds/korvin-merrow/task-setup/task2/pilot-run-clean/`: clean-pilot evidence folder. Contains score spread, representative output DOCX/PDF, low/high run DOCX exports, and grading transcript summary. Historical evidence only; do not upload from this folder.
- `worlds/korvin-merrow/task-setup/task2/KM02-escalation-results.md`: Task 2 escalation spread and fairness summary. Records 9 scored trajectories averaging about 65.1, the run-7 0.35 propagation failure, run-8 no-score tooling failure, and the ship-vs-soften question for human review.
- `worlds/korvin-merrow/task-setup/task2/KM02-learnings.md`: Task 2 retrospective. Promotes the forced-slot difficulty lesson toward backbone doctrine, records the mounted adversarial-input pattern, and preserves process gotchas for Tasks 3-6.
- `worlds/korvin-merrow/task-setup/task2/KM02-taiga-qa-log.md`: Taiga QA log for the escalation run. Records Env Linter / Data Quality outcomes and the correction that the `enable_anthropic_api` annotation field must contain exactly `tech issue`; reasoning belongs only in Slack or the dedicated dismissal-reasoning field.
- `worlds/korvin-merrow/task-setup/task2/FA-GA-current.md`: prepared KM02 FA/GA prose candidate for run 7 / score 0.35. Do not enter until the platform FA/GA gate is opened by Alexander.
- `worlds/korvin-merrow/task-setup/task2/escalation-run/`: escalation-run evidence folder. Contains representative catch and propagation DOCX outputs, the run-7 grading transcript, and the run-8 no-score trajectory note. Evidence only; do not upload from this folder.
- `worlds/korvin-merrow/task-setup/task2/escalation/discharge_summary_draft_incomplete_05242026.docx`: canonical review/provenance copy of the colleague-draft escalation DOCX. Byte-identical to the platform escalation copy. It is an intentionally flawed draft for the next run, not a world file and not a final discharge summary.
...
- `worlds/korvin-merrow/task-setup/platform/task1/`: Task 1 live platform provenance folder. Active folder contains only `prompt-task1-v5.txt`, `golden-response-task1-v6.docx`, `grader-guidelines-task1-v10.txt`, `medication_safety_handoff_pharmacy_05232026.docx`, and README.
...
- `worlds/korvin-merrow/task-setup/platform/task2/`: Task 2 / KM02 platform-prep area. Clean-pilot set remains as historical platform-prep (`README.md`, `prompt-task2.txt`, `golden-KM02-v5.docx`, `grader-guidelines-task2.txt`, and `hold/discharge_summary_draft_incomplete.md`). Clean pilot used no mounted task file and was too easy. Task AutoQC passed after stripping the golden-only synthetic footer token; the finalized world files were not changed.
- `worlds/korvin-merrow/task-setup/platform/task2/escalation/`: Task 2 escalation platform set. Contains `README.md`, `prompt-task2-escalation.txt`, and `discharge_summary_draft_incomplete_05242026.docx`. Used for the Alexander-operated Task-Writing Go-Back escalation run; golden and grader remained unchanged from the clean-pilot set.
...
- Task 1 final human review is complete. Task 2 clean pilot was too easy and the escalation run produced a usable failure spread. Current gate is Taiga QA Feedback AutoQC / QA handling, then FA/GA if cleared; do not run additional task uploads, additional agent runs, additional QA runs, create AutoQC responses, create scoring artifacts, or mutate RL Studio beyond the exact authorized step without explicit Alexander authorization.
