...
22. `worlds/korvin-merrow/task-setup/task5/build-phase-drafts/` for KM05 copied inputs, byte/source audit, draft prompt, golden/grader deltas, mounted-source draft, manifest plan, pilot preregistration, and Claude review request only.
23. `worlds/korvin-merrow/task-setup/platform/task1/` for live Task 1 platform artifacts.
24. `worlds/korvin-merrow/task-setup/platform/task1/archive/` only for historical Task 1 iterations.
...
6/7 late: KM05 PREBUILD REVIEW PACKET STAGED ONLY. Start at task5/TASK5-STATE.md, task5/design/KM05-design-plan-for-review.md, task5/KM05-prebuild-review-and-build-gates.md, and task5/build-phase-drafts/KM05-review-request-for-claude-ai.md. Proposed mechanism: unsigned transition-clinic pre-chart draft for 05/31 that overstates unverified +7 interval status from pending inpatient discharge plans. NOT performed by Codex: DOCX build, platform-current staging, RL Studio upload, Task AutoQC, Taiga trajectories, QA, or platform mutation.
