...
Purpose: Step 10 task setup preparation area after `Healthcare_247_Merrow` world creation. This folder is for local review packets, de-hinting maps, and Alexander-ruling prompts before any RL Studio task mutation.
Path: `worlds/korvin-merrow/task-setup/`.
Status: active local preparation only.
Current contents:
...
Boundaries:
- This is not an RL Studio upload record.
- Do not edit locked TP/EO/Golden/GG artifacts from this folder unless Alexander explicitly authorizes the exact change.
- Do not create platform-ready export packages, run agents, run QA, create AutoQC responses, create scoring artifacts, or mutate RL Studio from this folder without explicit authorization.
