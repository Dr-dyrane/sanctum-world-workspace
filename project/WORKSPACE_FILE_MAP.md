...
20. `worlds/korvin-merrow/task-setup/task3/fa-ga/FA-GA-current.md` for the verified local KM03 v2.2 FA/GA current draft on Attempt 9 (0.20), with Attempt 8 (0.97) as GA comparator. It is physician-owned and not platform-entered.
...
24. `worlds/korvin-merrow/task-setup/task4/TASK4-STATE.md` for KM04 v1 difficulty-failed state, v2 planning-review state, boundaries, and next gates.
...
26. `worlds/korvin-merrow/task-setup/platform/task4/current/` for the KM04 v1 platform set as evidence only. It is not a shipping candidate after the 6/8 difficulty failure unless Alexander explicitly overrides.
27. `worlds/korvin-merrow/task-setup/task4/build-phase-drafts/KM04-v2-plan.md` for the current review-only KM04 v2 redesign plan.
28. `worlds/korvin-merrow/task-setup/task4/build-phase-drafts/KM04-v2-review-request-for-claude-ai.md` for the paired self-contained Claude.ai red-team request.
29. `worlds/korvin-merrow/task-setup/task4/build-phase-drafts/KM04-v2-build-proposal.md` for the review-only draft build proposal. It is not a final platform artifact or build authorization.
30. `worlds/korvin-merrow/task-setup/task4/build-v2/` for the raw local KM04 v2 build set. Do not upload from this folder; use `platform/task4/current/` after Alexander sign-off.
31. `worlds/korvin-merrow/task-setup/task4/build-phase-drafts/KM04-codex-black-team-review-6-7.md` for the Codex black-team verdict before trajectory evidence superseded the difficulty prediction.
32. `worlds/korvin-merrow/task-setup/task4/design/KM04-design-plan-for-review.md` plus `task4/KM04-prebuild-review-and-build-gates.md` for KM04 mechanism history.
33. `worlds/korvin-merrow/task-setup/task4/build-phase-drafts/` for KM04 draft review material, Claude.ai review, Codex review, copied inputs, and v2 planning only.
34. `worlds/korvin-merrow/task-setup/task5/TASK5-STATE.md` for KM05 review-only state, source chain, boundaries, and next gates.
35. `worlds/korvin-merrow/task-setup/task5/design/KM05-design-plan-for-review.md` plus `task5/KM05-prebuild-review-and-build-gates.md` for KM05 mechanism history and gate checklist.
36. `worlds/korvin-merrow/task-setup/task5/design/KM05-claude-ai-proposal-6-7.md` and `worlds/korvin-merrow/task-setup/task5/design/KM05-codex-black-team-6-7.md` for KM05 review convergence, hold recommendation, and clean-baseline-versus-escalation decision surface.
37. `worlds/korvin-merrow/task-setup/task5/build-phase-drafts/` for KM05 copied inputs, byte/source audit, draft prompt, golden/grader deltas, mounted-source draft, manifest plan, pilot preregistration, and Claude review request only.
38. `worlds/korvin-merrow/task-setup/platform/task1/` for live Task 1 platform artifacts.
39. `worlds/korvin-merrow/task-setup/platform/task1/archive/` only for historical Task 1 iterations.
...
Status: Task 1 final human review complete / approved. Task 1 AO rework, hardening, Abi pre-check, revised platform entry, pilot runs, FA/GA, Preference Labeling, and final review are complete. Task 2 / KM02 is COMPLETE / RFD (Ready for Delivery) after Janette's 6/8 final review (post reseed/date fix); Task AutoQC / Taiga gates passed qcaud_5e, qcaud_4a, and qcaud_ef, final FA/GA uses Attempt 8 at 0.30 from v3 job `8f393839`, and submitted PL evidence lives under `task2/preference-labeling/`. KM03 v2.1 is recorded as difficulty-failed after job `58b5f3e3`; active KM03 v2.2 platform files are in `platform/task3/current/`, Task AutoQC passed 36/36 (`qcaud_fc`), and Taiga job `877aa204` cleared the difficulty gate. KM03 grading transcripts are verified clean, the local FA/GA drafting packet exists, and the verified local FA/GA current draft is under `task3/fa-ga/`; next eligible platform step is FA/GA entry on Attempt 9. KM04 v1 completed trajectory evidence on 6/8 and failed the difficulty gate: job `55ee209f`, 10 runs 0.87-0.95, mean 0.912, zero sub-70. KM05 has local prebuild review convergence under `task5/` and remains held before any build, platform staging, upload, AutoQC, agent run, or QA. Further platform actions remain under Alexander's direct operation. Task 6 remains held unless Alexander explicitly authorizes bounded parallel prep or the next pod-directed task step.
Current contents:
...
- `worlds/korvin-merrow/task-setup/task3/fa-ga/FA-GA-current.md`: Verified local KM03 v2.2 FA/GA current draft for Attempt 9 (0.20), with Attempt 8 (0.97) as GA comparator. It is physician-owned and not platform-entered; no FA/GA AutoQC, PL, final review, upload, or RL Studio mutation is implied.
...
- `worlds/korvin-merrow/task-setup/task4/TASK4-STATE.md`: Task 4 cockpit. Records KM04 v1 trajectory difficulty failure, staged v1 evidence set, deterministic source chain, consultant-consensus overclaim mechanism, Codex black-team status, and redesign/hold boundaries.
...
- `worlds/korvin-merrow/task-setup/platform/task4/current/`: KM04 platform-current local staging folder. Contains the v2 staged set (`prompt-task4-v2.txt`, `interdisciplinary_care_plan_draft_05242026.docx`, `golden-KM04-v2.docx`, `grader-guidelines-task4-v2.txt`) plus current `RUN-INSTRUCTIONS.md`. Read-only check confirmed golden v2 has no bracketed placeholder and no DOCX core-metadata leak, but it still requires Alexander physician review/sign-off before upload. No upload, AutoQC, Taiga, FA/GA, PL, or final review exists.
...
- `worlds/korvin-merrow/task-setup/task5/TASK5-STATE.md`: Task 5 cockpit. Records the deterministic KM05 source chain, source audit, proposed early post-discharge follow-up escalation, review gates, and no-build/platform boundaries.
...
- `worlds/korvin-merrow/task-setup/task5/design/KM05-claude-ai-proposal-6-7.md`: Claude.ai KM05 proposal review. Verifies the chart closes at 05/24, frames KM05 as fair but structurally weaker, and recommends hold with staged baseline/escalation decision.
- `worlds/korvin-merrow/task-setup/task5/design/KM05-codex-black-team-6-7.md`: Codex KM05 black-team review. Confirms conceptually GO / HOLD, moderate evidence-boundary classification, and the open clean-baseline-versus-straight-to-escalation decision.
...
6/8: KM04 v1 DIFFICULTY FAILED. Alexander-operated Run All QA / trajectory job `55ee209f-c9fa-4a64-8071-70d8917508da` completed with scores 0.87-0.95, mean 0.912, zero sub-70. Durable record: task4/runs/KM04-taiga-results-55ee209f.md. The platform/task4/current set is v1 evidence after failure, not a shipping candidate. Do not advance to FA/GA, Preference Labeling, final review, rerun, or redesign without explicit Alexander authorization.

6/7 late: KM05 PREBUILD REVIEW CONVERGED ONLY. Start at task5/TASK5-STATE.md, task5/design/KM05-design-plan-for-review.md, task5/design/KM05-claude-ai-proposal-6-7.md, task5/design/KM05-codex-black-team-6-7.md, task5/KM05-prebuild-review-and-build-gates.md, and task5/build-phase-drafts/KM05-review-request-for-claude-ai.md. Proposed mechanism: unsigned transition-clinic pre-chart draft for 05/31 that overstates unverified +7 interval status from pending inpatient discharge plans. Open Alexander decision: clean baseline versus straight-to-escalation after KM03/KM04 and gates G0-G8. NOT performed by Codex: DOCX build, platform-current staging, RL Studio upload, Task AutoQC, Taiga trajectories, QA, or platform mutation.
