...
12. `worlds/korvin-merrow/task-setup/platform/task3/current/` for the KM03 active v2.1 platform set uploaded by Alexander; Task AutoQC passed (`qcaud_fc`) and Taiga is intentionally held.
...
15. `worlds/korvin-merrow/task-setup/task4/TASK4-STATE.md` for KM04 staged-set state, boundaries, and next gates.
16. `worlds/korvin-merrow/task-setup/platform/task4/current/` for the KM04 locally staged platform set. It is not uploaded, AutoQC-run, or agent-run.
17. `worlds/korvin-merrow/task-setup/task4/build-phase-drafts/KM04-codex-black-team-review-6-7.md` for the Codex black-team verdict before final platform use.
18. `worlds/korvin-merrow/task-setup/task4/design/KM04-design-plan-for-review.md` plus `task4/KM04-prebuild-review-and-build-gates.md` for KM04 mechanism history.
19. `worlds/korvin-merrow/task-setup/task4/build-phase-drafts/` for KM04 draft review material, Claude.ai review, Codex review, and copied inputs only.
20. `worlds/korvin-merrow/task-setup/task5/TASK5-STATE.md` for KM05 review-only state, source chain, boundaries, and next gates.
21. `worlds/korvin-merrow/task-setup/task5/design/KM05-design-plan-for-review.md` plus `task5/KM05-prebuild-review-and-build-gates.md` for KM05 mechanism history and gate checklist.
22. `worlds/korvin-merrow/task-setup/task5/design/KM05-claude-ai-proposal-6-7.md` and `worlds/korvin-merrow/task-setup/task5/design/KM05-codex-black-team-6-7.md` for KM05 review convergence, hold recommendation, and clean-baseline-versus-escalation decision surface.
23. `worlds/korvin-merrow/task-setup/task5/build-phase-drafts/` for KM05 copied inputs, byte/source audit, draft prompt, golden/grader deltas, mounted-source draft, manifest plan, pilot preregistration, and Claude review request only.
24. `worlds/korvin-merrow/task-setup/platform/task1/` for live Task 1 platform artifacts.
25. `worlds/korvin-merrow/task-setup/platform/task1/archive/` only for historical Task 1 iterations.
...
- `worlds/korvin-merrow/task-setup/task3/TASK3-STATE.md`: Task 3 current-state cockpit. KM03 v2.1 is the active platform set; Alexander uploaded it and Task AutoQC passed with no non-pass flags (`qcaud_fc`); Taiga trajectories are intentionally held. Open: Alexander physician sign-off on v2.1 golden clinical content.
- `worlds/korvin-merrow/task-setup/task3/KM03-state-log.md`: Task 3 live event log for build, AutoQC, and run-state changes. Current event: v2.1 active platform set uploaded; Task AutoQC passed (`qcaud_fc`); Taiga held. Retired v1 `qcaud_6b` pass is historical only.
...
- `worlds/korvin-merrow/task-setup/platform/task3/current/`: KM03 active v2.1 platform set. Contains writer-owned prompt `prompt-task3-v2.txt`, mounted draft `care_coordination_handoff_draft_05242026.docx` (`f3b7bcdf`), golden `golden-KM03-v2.docx` (`3da7386f`, draft sign-off pending), grader `grader-guidelines-task3-v2.txt`, and local `RUN-INSTRUCTIONS.md`. Task AutoQC passed with no non-pass flags (`qcaud_fc`); Taiga trajectories are intentionally held; do not rerun AutoQC, rerun agents, rerun QA, upload additional files, or mutate RL Studio without exact Alexander authorization.
- `worlds/korvin-merrow/task-setup/task4/TASK4-STATE.md`: Task 4 cockpit. Records staged platform set, deterministic source chain, consultant-consensus overclaim design direction, Codex black-team status, and upload/AutoQC boundaries.
...
- `worlds/korvin-merrow/task-setup/platform/task4/current/`: KM04 locally staged platform set. Contains upload candidates `prompt-task4-escalation.txt`, `interdisciplinary_consultant_synthesis_draft_05242026.docx`, `golden-KM04-v1.docx`, `grader-guidelines-task4.txt`, and local `RUN-INSTRUCTIONS.md`. Not uploaded, not AutoQC-run, not agent-run; golden requires Alexander physician sign-off before final platform use.
...
- Task 1 final human review is complete. Task 2 / KM02 is COMPLETE / RFD (Ready for Delivery) after passing qcaud_5e, qcaud_4a, qcaud_ef, Preference Labeling with submitted verdict B / B++, and Janette's 6/8 final review; KM03 v2.1 Task AutoQC passed with no non-pass flags (`qcaud_fc`); Taiga trajectories are intentionally held; KM04 is locally staged and black-team passed but not uploaded, AutoQC-run, or agent-run; KM05 is review-converged and not build-ready. Do not run additional task uploads, additional agent runs, additional QA runs, create AutoQC responses, create scoring artifacts, resubmit preference labels, alter KM04 platform-current files, build or stage KM05 platform-current files, or mutate RL Studio beyond the exact authorized step without explicit Alexander authorization.
...
6/7 late: KM04 LOCAL STAGED SET READY FOR ALEXANDER SIGN-OFF / PLATFORM OPERATION. Current set is platform/task4/current/ (prompt-task4-escalation.txt, interdisciplinary_consultant_synthesis_draft_05242026.docx, golden-KM04-v1.docx DRAFT sign-off-pending, grader-guidelines-task4.txt, RUN-INSTRUCTIONS.md). Codex black-team review passed with no blocker at task4/build-phase-drafts/KM04-codex-black-team-review-6-7.md. NOT performed by Codex: RL Studio upload, Task AutoQC, Taiga trajectories, QA, or platform mutation. Remaining local gate: Alexander physician sign-off on golden-KM04-v1.docx before final platform use.
