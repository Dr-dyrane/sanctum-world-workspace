...
KM05 update 6/8: review-only reset / HOLD. Start with `worlds/korvin-merrow/task-setup/task5/TASK5-STATE.md` and `worlds/korvin-merrow/task-setup/task5/design/KM05-v2-design-plan-6-8.md`; older 6/7 convergence files are historical. Current lead hypothesis is a single unsupported post-discharge BMP / stable renal-potassium monitoring claim inside a transition-clinic draft. The older moderate-task classification is retired by the 6/8 no-moderate directive; future KM05 must target a real clinical failure. No KM05 DOCX build, platform-current staging, RL Studio upload, Task AutoQC, Taiga trajectories, QA, or platform mutation has occurred.
