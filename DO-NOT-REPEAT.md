...
Last updated: 2026-06-11. Sources distilled here: `docs/task-difficulty-lessons.md`, `docs/task-structure-dossier.md`, `docs/grader-guidelines-lessons.md`, `docs/clinical-voice-lessons.md`, `docs/reviewer-response-protocol.md`, `docs/git-workflow.md`, `TASK-RUNBOOK.md`, and the `KM-vs-QUILL-SANCTUM-MAPPING-REVIEW.md`. None of these are new mistakes; they are the ones we already paid for.
...
- Planted a false claim in the model's OWN draft with no instruction to correct it, then floored the model for propagating it (KM05, KM06, KM07 v2 - retired as unfair). FIX: do one of (a) re-attribute the claim to an unverified source, (b) leave a placeholder the model synthesizes from the chart, or (c) a reconcile-and-correct prompt instruction. (`docs/task-difficulty-lessons.md` sections 5-6)
